...
def run_impact_analysis(df_ips, weather_features, outage_features):
    ...
    for index, row in df_ips.iterrows():
        ...
        if pd.notnull(row['lat']):
            ...
            zones = get_point_zones(row['lat'], row['lon'])
            p_forecast = zones['forecastZone'].split('/')[-1] if zones.get('forecastZone') else ''
            p_county = zones['county'].split('/')[-1] if zones.get('county') else ''
            
            for feature in weather_features:
                geom = feature.get('geometry')
                props = feature.get('properties', {})
                event_name = props.get('event') or props.get('prod_type') or props.get('ps') or "Weather Alert"
                
                # Optional: Uncomment to filter by severity threshold
                # if props.get('severity', 'Unknown') not in ['Extreme', 'Severe']:
                #     continue
                ...
                if geom:
                    try:
                        ...
                        if poly.contains(point):
                            is_at_risk = True
                            hazards.append(event_name)
                    except:
                        pass
                else:
                    ugc = props.get('geocode', {}).get('UGC', [])
                    if ugc and (p_forecast in ugc or p_county in ugc):
                        is_at_risk = True
                        hazards.append(event_name)
